...
order, resolving paths relative to the project root rather than the cross_ai/
subdirectory.
"""
import os
import sys
...
# ── Path constants ─────────────────────────────────────────────────────────────
# _CROSS_AI_DIR  — absolute path to the cross_ai/ package directory
#                  (where mmd_startup.py and all st-*.py live)
# _PROJECT_ROOT  — parent of cross_ai/; the repo root for dev installs,
#                  site-packages/ for pip installs (no .env there — no-op).
_CROSS_AI_DIR = os.path.dirname(os.path.realpath(__file__))
_PROJECT_ROOT = os.path.dirname(_CROSS_AI_DIR)
...
def require_config() -> None:
    ...
    Checks the same three locations as the A1 load order:
      ...
      2b. <cross_ai-dir>/.env — co-located keys (future layout / pip installs)
    ...
    local_env2 = os.path.join(_CROSS_AI_DIR, ".env")   # cross_ai/.env  (future)
...
def load_cross_env() -> None:
    ...
    via runpy.run_path(), or directly as ``python cross_ai/st-*.py``.

    Layer order (first writer of each key wins; CWD overrides all):
      1. ~/.crossenv                — global config / API keys
      2. <project-root>/.env        — repo-local developer keys (cross/.env)
      2b. <cross_ai-dir>/.env       — co-located .env (future / pip layout)
    ...
    load_dotenv(os.path.join(_CROSS_AI_DIR, ".env"))               # 2b. cross_ai/
